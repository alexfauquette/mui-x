...
const tooltipItemPositionGetter: TooltipItemPositionGetter<'sankey'> = (params) => {
  const { series, identifier, placement } = params;
  ...
  if (identifier.subType === 'node') {
    const node = seriesItem.sankeyLayout.nodes.find((n) => n.id === identifier.nodeId);
  ...
  if (identifier.subType === 'link') {
    const link = seriesItem.sankeyLayout.links.find(
