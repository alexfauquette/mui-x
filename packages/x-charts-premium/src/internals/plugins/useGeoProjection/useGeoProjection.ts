...
export const useGeoProjection: ChartPlugin<UseGeoProjectionSignature> = ({ params, store }) => {
  const { geoData, featuresName, projection, translate, rotate, scale } = params;
  ...
  React.useEffect(() => {
    ...
    store.set('geoProjection', {
      ...
      rotate: rotate ?? null,
      scale: scale ?? null,
      featuresName: featuresName ?? undefined,
    });
  }, [geoData, featuresName, projection, translate, rotate, scale, store]);
...
useGeoProjection.getInitialState = (params) => ({
  geoProjection: {
    ...
    featuresName: params.featuresName ?? undefined,
